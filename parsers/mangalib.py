...
if __name__ == "__main__":
    ...
    def check_manga_exists(url: str) -> bool:
        with DatabaseHandler() as db:
            return db.select(
                table=tables.Manga,
                filters=(
                    tables.Manga.url == url,
                )
            )
    ...
    def add_chapter(data: dict[Any, Any]):
        with DatabaseHandler() as db:
            db.insert(
                table=tables.Chapter,
                data=data
            )


    def check_chapter_exists(url: str) -> bool:
        with DatabaseHandler() as db:
            return db.select(
                table=tables.Chapter,
                filters=(
                    tables.Chapter.url == url,
                )
            )
